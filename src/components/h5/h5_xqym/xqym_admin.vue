        ...
        <van-row class="BOXshadow mT bg" type="flex">
          ...
          <van-col span="24">
            ...
            <van-field
                    ...
                    label="预计有车时间"
                    placeholder="点击选择时间"
                    @click="showyjycsj = true"
                    left-icon="todo-list-o"
                  />
                  <van-popup v-model="showyjycsj" position="bottom">
                    <van-datetime-picker
                      type="datetime"
                      title="选择年月日小时"
                      :min-date="minDate"
                      :formatter="formatter"
                      :max-date="maxDate"
                      @confirm="onshowyjycsj"
                      @cancel="showyjycsj = false"
                    />
                  </van-popup>
...
export default {
  ...
  methods: {
    onshowyjycsj(time){  //预计有车时间
      ...
      this.mydate[0].yjycsj = youWant;
      this.showyjycsj = false;
    },
  ...
  created() {
    api_getAllUser().then((res) => {
      let data2 = this.convertKey(res.data, [
        "id",
        "password",
        "userStatus",
        "userType",
